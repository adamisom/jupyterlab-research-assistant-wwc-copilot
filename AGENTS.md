...
After editing Python files (like `jupyterlab_research_assistant_wwc_copilot/routes.py`):

```bash
python -m py_compile jupyterlab_research_assistant_wwc_copilot/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `jupyterlab_research_assistant_wwc_copilot/*.py` files):
...
- Backend processing in `jupyterlab_research_assistant_wwc_copilot/routes.py`
...
- **✅ Do**: `jupyterlab_research_assistant_wwc_copilot/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'jupyterlab_research_assistant_wwc_copilot:plugin';
```
...
const plugin: JupyterFrontEndPlugin<void> = {
  id: 'jupyterlab_research_assistant_wwc_copilot:plugin',
  autoStart: true,
...
- Backend routes: `jupyterlab_research_assistant_wwc_copilot/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `jupyterlab_research_assistant_wwc_copilot/` directory
...
1. **Read the backend first** — Check `jupyterlab_research_assistant_wwc_copilot/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `jupyterlab_research_assistant_wwc_copilot/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "jupyterlab_research_assistant_wwc_copilot", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `jupyterlab_research_assistant_wwc_copilot/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
**After editing Python** (files in `jupyterlab_research_assistant_wwc_copilot/`):
...
- Ask user to search for the extension ID (`jupyterlab_research_assistant_wwc_copilot`) to see if it loaded
...
ls -la jupyterlab_research_assistant_wwc_copilot/labextension/  # Should contain bundled extension
```
...
- Backend routes: `jupyterlab_research_assistant_wwc_copilot/routes.py`
- Server extension config: `jupyter-config/server-config/jupyterlab_research_assistant_wwc_copilot.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `jupyterlab_research_assistant_wwc_copilot/routes.py`
...
```css
.jp-jupyterlab-research-assistant-wwc-copilot-widget {
  padding: 8px;
}
```
...
**❌ Don't**: Use absolute imports like `from jupyterlab_research_assistant_wwc_copilot.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'jupyterlab_research_assistant_wwc_copilot:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyterlab_research_assistant_wwc_copilot:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyterlab-research-assistant-wwc-copilot-ClassName`
- **API routes** (in `jupyterlab_research_assistant_wwc_copilot/routes.py`): `url_path_join(base_url, "jupyterlab_research_assistant_wwc_copilot", "endpoint")`
